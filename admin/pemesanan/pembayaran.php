...
if (isset($_POST['simpan_pembayaran'])) {
    ...
    $sisa = max(0, (float)$data['total'] - $sudah);
    if ($jumlah <= 0) $error = 'Jumlah pembayaran harus lebih dari 0.';
    elseif ($jumlah > $sisa + 0.009) $error = 'Jumlah pembayaran melebihi sisa tagihan.';
    else {
        // Status pembayaran pada tabel mengikuti total pembayaran booking.
        $status_bayar = ($sudah + $jumlah >= (float)$data['total'] - 0.009) ? 'lunas' : 'belum_lunas';
...
$terbayar=total_terbayar($con,$id_booking); $sisa=max(0,(float)$data['total']-$terbayar); $sp=status_pembayaran((float)$data['total'],$terbayar);
// Sinkronkan status pada data pembayaran lama/import agar tidak menyisakan ENUM kosong.
...
<?php if ($sisa > 0): ?><form method="POST"><div class="mb-3"><label class="form-label">Jumlah Bayar</label><input type="number" name="jumlah_bayar" class="form-control" min="1" max="<?= $sisa; ?>" step="0.01" required></div><div class="mb-3"><label class="form-label">Metode Pembayaran</label><select name="metode" class="form-select"><option>Cash</option><option>Transfer</option><option>QRIS</option><option>Debit</option><option>Kartu Kredit</option></select></div><div class="mb-3"><label class="form-label">Status</label><input class="form-control" value="Otomatis berdasarkan total pembayaran" readonly></div><button name="simpan_pembayaran" class="btn btn-warning w-100"><i class="fas fa-save me-1"></i>Simpan Pembayaran</button></form><?php else: ?><div class="alert alert-success mb-0">Tagihan sudah lunas.</div><?php endif; ?><a href="detail.php?id=<?= $id_booking; ?>" class="btn btn-secondary w-100 mt-2">Kembali ke Detail</a></div></div></div>
