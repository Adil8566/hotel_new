...
if (!$b || !in_array(strtolower(trim($b['status'])), ['pending','dikonfirmasi'], true)) { header('Location: pemesanan.php'); exit; }
mysqli_query($con, "UPDATE tbl_booking SET status='dibatalkan' WHERE id_booking={$id}");
sync_single_room_status($con, (int)$b['id_kamar']);
header('Location: pemesanan.php?pesan=batal'); exit;
?>